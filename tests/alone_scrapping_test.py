...
def find_by(param_name, param_text, dr):
    try:
        ...
        #     return dr.find_elements_by_tag_name(param_text)[7].text
        ...
        if param_name == 'class':
            ...
            # a = 0
            # b = 1
            # to = dr.find_elements_by_tag_name(param_text)
            # a = 0
            # b = 1
            # ot = to[7].get_attribute('textContent')
            # return ot
        ...
        # find_by(param_name, param_text, dr)
...
def scr_user(dr, number_user, link_to_user, region_code):
    global GRAY_USERS, WHITE_USERS, BLACK_USERS, PATH_TO_USERS_INFORMATION
    dr.get(link_to_user)
    try:
...
def load_proxy(path=PATH_TO_PROXY):
    with open(path, "r") as Fi:
        proxies = set(Fi.readlines())
    proxies = list(proxies)
    proxies = [this.strip() for this in proxies]
    information('{} proxies load successfully.'.format(len(proxies)))
    return proxies


def save_photo(photo_inform):
    global ALL_PROXY
    u_id = photo_inform[0]
    ...
    if to_save.status_code == 404:
        to_save = requests.get(photo_link, proxies=to_get)
    global PATH_TO_PHOTOS, PATH_TO_PHOTOS_LINKS
    photo_name = PATH_TO_PHOTOS + '/{}_{}.jpg'.format(u_id, number)  # user_id + photo number
    out = open(photo_name, "wb")
    out.write(to_save.content)
    out.close()
    my_csv.csv_line_writer(PATH_TO_PHOTOS_LINKS, [number, photo_link])
    pass


def main():
    information('Start program.')
    global PATH_TO_WEBDRIVER, GRAY_USERS, TIME_TO_WAIT, ALL_PROXY
    user_links, users_code = load_user_links()
    ALL_PROXY = load_proxy()
